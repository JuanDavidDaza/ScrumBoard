...
const updateUser = async (req, res) => {
  ...
  let user = await User.findOneAndUpdate(req.body._id, {
    name: req.body.name,
    email: req.body.email,
    password: pass,
    roleId: req.body.roleId,
  ...
  if (!user) return res.status(400).send("Process failed: editing user");

  return res.status(200).send({ user });
};

const deleteUser = async (req, res) => {
  if(!req.body._id) return res.status(400).send("Process Failed: incomplete data");
